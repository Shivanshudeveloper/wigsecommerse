...
const CartItemList = ({ deleteItem, addItem, allItem }) => {
  ...
  var photo2 = allItem?.product?.images[1];
  var photo3 = allItem?.product?.images[2];
  var photo4 = allItem?.product?.images[3];
    ...
    <>
      <Grid item xs={3}>
        <Card className={classes.cardroot} variant="outlined">
          <CardContent>
            <IconButton
              onClick={() => deleteItem(allItem?._id)}
              color="primary"
            ...
            {allItem?.product.verified ? (
              <img
                style={{ float: "right" }}
                src="https://img.icons8.com/color/28/000000/verified-badge.png"
              />
            ) : null}
            <br />
            <br />
...
const WishList = () => {
  ...
  React.useEffect(() => {
    ...
    axios
      .get(`${API_SERVICE}/api/v1/main/findallwishlishtcartitems/${uid}`)
      .then((response) => {
  ...
  const deleteItem = (documentId) => {
    ...
    var uid = sessionStorage.getItem("userId");
    axios
      .get(`${API_SERVICE}/api/v1/main/removeitemwishlisttocart/${documentId}`)
      .then((response) => {
  ...
  const searchProducts = async () => {
    ...
    axios
      .get(
        `${API_SERVICE}/api/v1/main/findsearchwishlishtcartitems/${uid}/${searchValue}`
      )
      .then((response) => {
        setAllItems(response.data);
        setLoading(false);
