...
const ViewProducts = () => {
  ...
  const handleDelete = async (p) => {
    let prod = [...products];
    prod = prod.filter((e) => e._id !== p._id);
    const res = await axios.delete(
      `${API_SERVICE}/api/v1/main/deleteproduct/${p._id}`
    );
    setProducts(prod);
  };
  ...
  const viewProd = (p) => {
    setCurrentData({
      ...
      freereturn: `${p.freereturn}`,
      shipfrom: `${p.shipfrom}`,
      length: `${p.length}`,
      color: `${p.color}`,
      blackowned: `${p.blackowned}`,
      producttype: `${p.producttype}`,
      origin: `${p.origin}`,
      texture: `${p.texture}`,
      basematerial: `${p.basematerial}`,
      lacetype: `${p.lacetype}`,
      preplucked: `${p.preplucked}`,
      bleachedKnots: `${p.bleachedKnots}`,
      babyHairs: `${p.babyHairs}`,
      parting: `${p.parting}`,
      closureSize: `${p.closureSize}`,
      frontalSize: `${p.frontalSize}`,
      density: `${p.density}`,
      price: `${p.price}`,
    });
  };
